<template>
  <v-container class="">
    <v-row>
      <v-col>
        <div class="display-1">Edit {{ title }} subcategory</div>
      </v-col>
    </v-row>
    <v-row>
      <v-col cols="12" sm="3">
...
export default {
  ...
  methods: {
    create() {
      Inertia.post(
        `/admin/subcategory/update/${this.id}
      `,
        this.form,
        {
          forceFormData: true,
          ...
          onStart: () => {
            ...
            this.text = "Subcategory update in progress ...";
          },
          onSuccess: () => {
            this.timeout = 3000;
            this.color = "green darken-3";
            this.text = `${this.title} updated Successfully`;
            this.redirect = true;
            this.loading = false;
          },
          onError: () => {
